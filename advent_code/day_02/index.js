...
// const myArr = [ "1-3 a: abcde", "1-3 b: cdefg", "2-9 c: ccccccccc" ];

let reg = /[-|: ]\s*/;
let result = 0;
let count = 0;

function searchIterations(array, symbol, min, max) {
  --min;
  --max;

  if ((array[max] === symbol && array[min] !== symbol) || (array[min] === symbol && array[max] !== symbol)) {
    ++result;
  }
...
for (let j of myArr) {
  let splits = j.split(reg);

  let min = splits[0];
  let max = splits[1];
  let symbol = splits[2];
  let searchArray = splits[3].split("");
  // console.log(searchArray);

  searchIterations(searchArray, symbol, min, max);
}

console.log("Совпадений: " + result);
